...
export async function runPrompts(project: ProjectInfo, options: { backendOnly?: boolean } = {}): Promise<UserAnswers> {
  ...
  if (!backendOnly) {
    ...
    if (project.isNextJs) {
      // ── Next.js path — use generated PushProvider in layout.tsx ──────────
      logger.raw('  // 2. A PushProvider component was generated in your components/ folder.')
      logger.raw('  //    Wrap your root layout children with it:')
      logger.blank()
      logger.raw("  import { PushProvider } from '@/components/PushProvider'")
      logger.blank()
      logger.raw('  export default function RootLayout({ children }) {')
      logger.raw('    return (')
      logger.raw('      <html>')
      logger.raw('        <body>')
      logger.raw('          <PushProvider>')
      logger.raw('            {children}')
      logger.raw('          </PushProvider>')
      logger.raw('        </body>')
      logger.raw('      </html>')
      ...
      // ── React path — wrap App directly ────────────────────────────────────
      logger.raw('  // 2. Wrap your app root (src/App.tsx):')
