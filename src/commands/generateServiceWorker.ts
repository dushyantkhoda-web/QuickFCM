...
export async function generateServiceWorker(): Promise<void> {
  ...
  if (await fileExists(defaultSwPath)) {
    ...
    if (!existingContent.includes('Generated by pushfire')) {
      suggestedPath = `public/${SERVICE_WORKER_FALLBACK_FILENAME}`
      logger.warn(`⚠  ${SERVICE_WORKER_FILENAME} already exists and was not generated by pushfire.`)
  ...
  try {
    ...
    logger.error('   Try reinstalling pushfire: npm install -g pushfire')
    return
  }
  ...
  try {
    ...
    logger.info(`  1. Install package: npm install pushfire`)
    logger.info(`  2. Wrap your app: import { CustomPushProvider } from 'pushfire'`)
